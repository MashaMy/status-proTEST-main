...
<button class="btn btn-outline-light text-center" type="button"  onClick={ handlClick }>Рассчитай свой вариант<span>развернуть</span></button>
{show && dataPlus.map ((element => {
      const {id,service, price, vol,  unc, znak } = element;
           
      return (     
      
           <FormGroup key={id} class="form-check" controlId='fromBasicKvadrat'>
           <FormCheck  
           type='checkbox' 
           onChange={(e) => handleCheck(e, id)} 
            /> 
            <p> {service} { price} {vol} { unc} {znak} </p> 
          </FormGroup>
...
<button onClick= { checkedSum } type="button" class="btn btn-outline-light text-center m-2"> Рассчитать</button>
<button onClick={() => setTotalsum(null)} type="button" class="btn btn-outline-light text-center m-2 "> Очистить</button>
 </div>
    <div class = 'text-center' >
